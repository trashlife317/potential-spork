...
# O(1) Lookup for note indices
NOTE_TO_INDEX = {note: i for i, note in enumerate(NOTES)}

# Normalization map for common enharmonic equivalents (Flats to Sharps)
# We only need capitalized keys because we capitalize input before lookup.
NOTE_NORM_MAP = {
    'Db': 'C#', 'Eb': 'D#', 'Gb': 'F#', 'Ab': 'G#', 'Bb': 'A#',
}

# Pre-calculated MIDI note names for O(1) lookup
# MIDI 0 is C-1, MIDI 127 is G9
MIDI_NOTE_NAMES = [f"{NOTES[i % 12]}{(i // 12) - 1}" for i in range(128)]
...
def get_note_index(note_name):
    ...
    # Fast path for correct case
    if note_name in NOTE_TO_INDEX:
        return NOTE_TO_INDEX[note_name]

    # Normalize case
    note_name_cap = note_name.capitalize()

    # Handle enharmonics (e.g. Db -> C#)
    if note_name_cap in NOTE_NORM_MAP:
        note_name_cap = NOTE_NORM_MAP[note_name_cap]

    if note_name_cap in NOTE_TO_INDEX:
        return NOTE_TO_INDEX[note_name_cap]

    raise ValueError(f"Invalid note name: {note_name}")


def get_scale_notes(root_note, scale_type, start_octave=3, end_octave=5):
    """Returns list of MIDI numbers for the scale across specified octaves."""
    try:
    ...
    if scale_key not in SCALES:
        scale_key = 'minor'  # Default to minor
...
def get_note_name(midi_number):
    ...
    if 0 <= midi_number <= 127:
        return MIDI_NOTE_NAMES[midi_number]
    return "Unknown"

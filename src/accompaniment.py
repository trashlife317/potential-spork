...
class DrumGenerator:
    def __init__(self, tempo, style='trap'):
        self.tempo = tempo
        self.style = style

    def generate_pattern(self, length_bars=4):
        """
        Generates drum events based on style.
        ...
        for bar in range(length_bars):
            ...
            if self.style == 'trap':
                self._generate_trap_bar(events, bar_offset)
            elif self.style == 'boombap':
                self._generate_boombap_bar(events, bar_offset)
            elif self.style == 'lofi':
                self._generate_lofi_bar(events, bar_offset)
            else:
                 # Default to trap
                 self._generate_trap_bar(events, bar_offset)
    ...
    def _generate_trap_bar(self, events, bar_offset):
        # 1. Hi-Hats (8th notes with rolls)
        for i in range(8):
            beat = i * 0.5
        ...
        # 2. Snare / Clap (Beat 3) - Half time feel common in Trap
        events.append({
            'note': 38, # Snare
            'duration': 1.0,
            'velocity': 127,
    ...
    def _generate_boombap_bar(self, events, bar_offset):
        # Boom Bap: Kick on 1, 1.5, 2.5, 3 etc. Snare on 2 and 4.
        # Swing feel on hi-hats.

        # 1. Hi-Hats (Eighth notes with slight velocity variation)
        for i in range(8):
            beat = i * 0.5
            # Swing: delay the off-beats (0.5, 1.5, 2.5, 3.5) slightly?
            # For MIDI grid, we'll just stick to straight 8ths but vary velocity heavily
            velocity = random.randint(90, 110) if i % 2 == 0 else random.randint(70, 90)
            events.append({
                'note': 42,
                'duration': 0.5,
                'velocity': velocity,
        ...
        # 2. Snare (Beat 2 and 4)
        for snare_beat in [1.0, 3.0]:
             events.append({
                'note': 38,
                'duration': 1.0,
                'velocity': random.randint(110, 127),
                'offset': bar_offset + snare_beat
            })

        # 3. Kick
        # Always on 1. Optional on "and of 2" (2.5), "and of 3" (3.5)
        kick_beats = [0.0]
        if random.random() < 0.6: kick_beats.append(2.5) # The "Boom-Bap" kick pattern often hits here
        if random.random() < 0.3: kick_beats.append(1.5)
        if random.random() < 0.2: kick_beats.append(3.5) # Pickup to next bar
    ...
    def _generate_lofi_bar(self, events, bar_offset):
        # Lofi: Simpler, lower velocity, more "lazy"
        # Snare on 2 and 4. Kick sparse.

        # 1. Hi-Hats (Sparse, maybe every beat or 8ths)
        for i in range(8):
             if random.random() < 0.8: # Skip some hats for lazy feel
                beat = i * 0.5
                events.append({
                    'note': 42,
                    'duration': 0.5,
                    'velocity': random.randint(50, 70), # Low velocity
                    'offset': bar_offset + beat
                })

        # 2. Snare / Rimshot (Beat 2 and 4)
        for snare_beat in [1.0, 3.0]:
             events.append({
                'note': 37, # Side Stick / Rimshot for Lofi? or just quiet Snare (38)
                'duration': 1.0,
                'velocity': random.randint(70, 90),
                'offset': bar_offset + snare_beat
            })
        ...
        # 3. Kick (Very simple)
        kick_beats = [0.0]
        if random.random() < 0.4: kick_beats.append(2.5)
